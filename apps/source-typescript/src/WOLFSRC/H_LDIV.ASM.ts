import { i32, u32 } from "./TS_C";

// @generated-from-wolfsrc
// Original file: source/WOLFSRC/H_LDIV.ASM
...
export const WOLFSRC_FILE = "H_LDIV.ASM";
...
export function LDIV(dividend: number, divisor: number): number {
  const rhs = i32(divisor);
  if (!rhs) {
    throw new RangeError("LDIV divide by zero");
  }
  return i32(Math.trunc(i32(dividend) / rhs));
}

export const F_LDIV = LDIV;
export const N_LDIV = LDIV;

export function LUDIV(dividend: number, divisor: number): number {
  const rhs = u32(divisor);
  if (!rhs) {
    throw new RangeError("LUDIV divide by zero");
  }
  return u32(Math.trunc(u32(dividend) / rhs));
}

export const F_LUDIV = LUDIV;
export const N_LUDIV = LUDIV;

export function LMOD(dividend: number, divisor: number): number {
  const rhs = i32(divisor);
  if (!rhs) {
    throw new RangeError("LMOD divide by zero");
  }
  const lhs = i32(dividend);
  return i32(lhs - LDIV(lhs, rhs) * rhs);
}

export const F_LMOD = LMOD;
export const N_LMOD = LMOD;

export function LUMOD(dividend: number, divisor: number): number {
  const rhs = u32(divisor);
  if (!rhs) {
    throw new RangeError("LUMOD divide by zero");
  }
  return u32(u32(dividend) - LUDIV(dividend, rhs) * rhs);
}

export const F_LUMOD = LUMOD;
export const N_LUMOD = LUMOD;
