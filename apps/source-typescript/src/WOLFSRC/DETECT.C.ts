import { SoundBlasterPresent } from "./ID_SD.C";

// @generated-from-wolfsrc
// Original file: source/WOLFSRC/DETECT.C
...
export const WOLFSRC_FILE = "DETECT.C";
export const WOLFSRC_FUNCTIONS = [
  "SDL_CheckSB",
  "SDL_DetectSoundBlaster"
] as const;

export let sbLocation = -1;

export function SDL_CheckSB(port: number): boolean {
  sbLocation = (Math.trunc(port) << 4) & 0xffff;
...
export function SDL_DetectSoundBlaster(port: number): boolean {
  let checkPort = Math.trunc(port);
  if (checkPort === 0) {
    checkPort = 2;
  }
