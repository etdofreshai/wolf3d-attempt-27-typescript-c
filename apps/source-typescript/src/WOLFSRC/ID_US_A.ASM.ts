...
//
// Reimplemented from semantics per PORTING.md §7; original assembly:
/*
;=================================================
;
 ...
 * if randomize is false, the counter is set to 0
 *
 * (randomize=true uses DOS GetSystemTime's hundredths byte & 0xff —
 *  determinism callers always pass false; the timer path takes a value
 *  from the platform clock.)
 */
export function US_InitRndT(randomize: boolean): void {
  if (!randomize) {
    us_a.rndindex = 0; // set to a definite value
  } else {
    us_a.rndindex = Math.floor(Date.now() / 10) & 0xff; // int 21h/2Ch dx low byte
  }
}
...
export function US_RndT(): number {
  us_a.rndindex = (us_a.rndindex + 1) & 0xff;
  return rndtable[us_a.rndindex];
}
