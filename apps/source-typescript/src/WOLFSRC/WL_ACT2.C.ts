...
import { STRUCT_LAYOUTS, nearOffsetForRuntimeSymbol, nearOffsetForSymbol } from "./TS_SAVE_LAYOUT";
import {
  A_SmokeMemory,
  CheckLineMemory,
  GetNewActorMemory,
  ...
  NUMAREAS,
  OpenDoorMemory,
  SpawnBJVictoryMemory,
  SpawnBossMemory,
  SpawnDeadGuardMemory,
  SpawnFakeHitlerMemory,
  SpawnFatMemory,
  SpawnGhostsMemory,
  SpawnGiftMemory,
  SpawnGretelMemory,
  ...
  SpawnSchabbsMemory,
  SpawnStandMemory,
  ProjectileTryMoveMemory,
  SelectChaseDirMemory,
  SelectDodgeDirMemory,
  SelectPathDirMemory,
  SelectRunDirMemory,
  T_BiteMemory,
  T_BJDoneMemory,
  ...
  type BJJumpSummary,
  type BJRunSummary,
  type BJVictorySpawnSummary,
  type ChaseStepSummary,
  type DamageSummary,
  type EnemySpawnOptions,
  type EnemySpawnSummary,
  type PathStepOptions,
  type PathStepSummary,
  type ProjectileSpawnSummary,
  type ProjectileStepSummary,
  type ShootSummary,
...
// Original file: source/WOLFSRC/WL_ACT2.C
...
export const WOLFSRC_FILE = "WL_ACT2.C";
...
const ANGELFIRESND = 69;
const KNIGHTMISSILESND = 78;
const GUARD_DEATH_SCREAMS = [
  DEATHSCREAM1SND,
  DEATHSCREAM2SND,
  DEATHSCREAM3SND,
  DEATHSCREAM4SND,
...
export interface UShootSummary {
  readonly shoot: ShootSummary;
  readonly dist: number;
  readonly touchDamage: DamageSummary | null;
}

export interface StartAttackSummary {
...
export interface HitlerMorphSummary {
  ...
  readonly actor: number;
  readonly state: "_s_hitlerchase1";
  readonly hitpoints: number;
  readonly speed: number;
}

export interface SoundActionSummary {
  readonly sound: number;
  readonly positioned: boolean;
  readonly actor: number | null;
}

export interface RelaunchSummary {
  readonly actor: number;
  readonly temp1: number;
...
export interface DormantSummary {
  ...
  readonly blockedBy: "player" | "wall" | "actor" | null;
  readonly state: string | null;
  readonly flags: number;
}

export interface StartDeathCamSummary {
  readonly actor: number;
...
export interface LaunchSummary extends ProjectileSpawnSummary {
  readonly sound: SoundActionSummary;
  readonly shoot: ShootSummary | null;
}

const localStateNames = new WeakMap<DOSMemory, Map<number, string>>();

function positionedSound(sound: number, actor: number): SoundActionSummary {
  return { sound, positioned: true, actor };
}

function globalSound(sound: number): SoundActionSummary {
  return { sound, positioned: false, actor: null };
}

export function A_Breathing(dgroup?: DOSMemory, actor?: number): SoundActionSummary | null {
  void dgroup;
  void actor;
  return null;
}

export function A_DeathScream(dgroup: DOSMemory, actor: number): SoundActionSummary | null {
  const obclass = dgroup.u16(actor + OBJ_CLASS_OFFSET);
  if (dgroup.u16(nearOffsetForRuntimeSymbol("_gamestate") + 2) === 9 && US_RndT() === 0) {
    switch (obclass) {
      case MUTANTOBJ:
      case GUARDOBJ:
      case OFFICEROBJ:
      case SSOBJ:
      case DOGOBJ:
        return positionedSound(DEATHSCREAM6SND, actor);
    }
  }

  switch (obclass) {
    case MUTANTOBJ:
      return positionedSound(AHHHGSND, actor);
    case GUARDOBJ:
      return positionedSound(GUARD_DEATH_SCREAMS[US_RndT() % GUARD_DEATH_SCREAMS.length], actor);
    case OFFICEROBJ:
      return positionedSound(NEINSOVASSND, actor);
    case SSOBJ:
      return positionedSound(LEBENSND, actor);
    case DOGOBJ:
      return positionedSound(DOGDEATHSND, actor);
    case BOSSOBJ:
      return globalSound(MUTTISND);
    case SCHABBOBJ:
      return globalSound(MEINGOTTSND);
    case FAKEOBJ:
      return globalSound(HITLERHASND);
    case MECHAHITLEROBJ:
      return globalSound(SCHEISTSND);
    case REALHITLEROBJ:
      return globalSound(EVASND);
    case GRETELOBJ:
      return globalSound(MEINSND);
    case GIFTOBJ:
      return globalSound(DONNERSND);
    case FATOBJ:
      return globalSound(ROSESND);
    default:
      return null;
  }
}

export function A_Dormant(dgroup: DOSMemory, actor: number): DormantSummary {
  ...
  if (deltax >= -MINACTORDIST && deltax <= MINACTORDIST) {
    ...
    if (deltay >= -MINACTORDIST && deltay <= MINACTORDIST) {
      return dormantSummary(dgroup, actor, false, "player");
    }
  }

  const xl = (dgroup.i32(actor + OBJ_X_OFFSET) - PLAYERSIZE) >> TILESHIFT;
  const xh = (dgroup.i32(actor + OBJ_X_OFFSET) + PLAYERSIZE) >> TILESHIFT;
  const yl = (dgroup.i32(actor + OBJ_Y_OFFSET) - PLAYERSIZE) >> TILESHIFT;
  const yh = (dgroup.i32(actor + OBJ_Y_OFFSET) + PLAYERSIZE) >> TILESHIFT;
  const actorat = nearOffsetForRuntimeSymbol("_actorat");
  for (let y = yl; y <= yh; y++) {
    for (let x = xl; x <= xh; x++) {
      if (x < 0 || y < 0 || x >= MAPSIZE || y >= MAPSIZE) {
        return dormantSummary(dgroup, actor, false, "wall");
      }
      const tile = dgroup.u16(actorat + (x * MAPSIZE + y) * 2);
      if (!tile) continue;
      if (tile === actor) continue;
      if (tile < 256) return dormantSummary(dgroup, actor, false, "wall");
      if (dgroup.u8(tile + OBJ_FLAGS_OFFSET) & FL_SHOOTABLE) {
        return dormantSummary(dgroup, actor, false, "actor");
      }
    }
  }
  ...
  dgroup.setU8(
    ...
    (dgroup.u8(actor + OBJ_FLAGS_OFFSET) | FL_AMBUSH | FL_SHOOTABLE) & ~FL_ATTACKMODE,
  );
  dgroup.setU16(actor + OBJ_DIR_OFFSET, NODIR);
  setStateName(dgroup, actor, "_s_spectrewait1");
  return dormantSummary(dgroup, actor, true, null);
}

export function A_HitlerMorph(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  actor: number,
): HitlerMorphSummary {
...
export function A_MechaSound(dgroup: DOSMemory, actor: number): SoundActionSummary | null {
  const area = dgroup.u8(actor + OBJ_AREANUMBER_OFFSET);
  if (area >= NUMAREAS || dgroup.u16(nearOffsetForRuntimeSymbol("_areabyplayer") + area * 2) === 0) {
    return null;
  }
  return positionedSound(MECHSTEPSND, actor);
}

export function A_Relaunch(dgroup: DOSMemory, actor: number): RelaunchSummary {
  const temp1 = dgroup.u16(actor + OBJ_TEMP1_OFFSET) + 1;
  dgroup.setU16(actor + OBJ_TEMP1_OFFSET, temp1);
  if (temp1 === 3) {
    setStateName(dgroup, actor, "_s_angeltired");
  } else if (US_RndT() & 1) {
    setStateName(dgroup, actor, "_s_angelchase1");
  }
  return { actor, temp1, state: stateName(dgroup, actor) };
}

export function A_Slurpie(dgroup?: DOSMemory, actor?: number): SoundActionSummary {
  void dgroup;
  void actor;
  return globalSound(SLURPIESND);
}

export function A_Smoke(dgroup: DOSMemory, actor: number): SmokeSummary {
  return A_SmokeMemory(dgroup, actor);
}

export function A_StartAttack(dgroup: DOSMemory, actor: number): StartAttackSummary {
  dgroup.setU16(actor + OBJ_TEMP1_OFFSET, 0);
  return { actor, temp1: dgroup.u16(actor + OBJ_TEMP1_OFFSET) };
}

export function A_StartDeathCam(dgroup: DOSMemory, actor: number): StartDeathCamSummary {
  const gamestate = nearOffsetForRuntimeSymbol("_gamestate");
  const playstate = nearOffsetForRuntimeSymbol("_playstate");
  const player = dgroup.u16(nearOffsetForRuntimeSymbol("_player"));
  if (dgroup.u16(gamestate + gamestateFieldOffset("victoryflag"))) {
    dgroup.setU16(playstate, EX_VICTORIOUS);
    return { actor, victorious: true, player, angle: dgroup.u16(player + OBJ_ANGLE_OFFSET), dist: 0, state: stateName(dgroup, actor) };
  }

  dgroup.setU16(gamestate + gamestateFieldOffset("victoryflag"), 1);
  setStateName(dgroup, player, "_s_deathcam");
  dgroup.setU32(player + OBJ_X_OFFSET, dgroup.u32(gamestate + gamestateFieldOffset("killx")));
  dgroup.setU32(player + OBJ_Y_OFFSET, dgroup.u32(gamestate + gamestateFieldOffset("killy")));

  const dx = dgroup.i32(actor + OBJ_X_OFFSET) - dgroup.i32(player + OBJ_X_OFFSET);
  const dy = dgroup.i32(player + OBJ_Y_OFFSET) - dgroup.i32(actor + OBJ_Y_OFFSET);
  const playerAngle = angleFromDelta(dx, dy);
  dgroup.setU16(player + OBJ_ANGLE_OFFSET, playerAngle);

  let dist = 0x14000;
  do {
    const radians = (playerAngle / ANGLES) * Math.PI * 2;
    const xmove = Math.trunc(dist * Math.cos(radians));
    const ymove = -Math.trunc(dist * Math.sin(radians));
    dgroup.setU32(player + OBJ_X_OFFSET, dgroup.i32(actor + OBJ_X_OFFSET) - xmove);
    dgroup.setU32(player + OBJ_Y_OFFSET, dgroup.i32(actor + OBJ_Y_OFFSET) - ymove);
    dist += 0x1000;
  } while (!CheckPosition(dgroup, player) && dist < 0x40000);

  dgroup.setU16(nearOffsetForRuntimeSymbol("_plux"), dgroup.u32(player + OBJ_X_OFFSET) >>> UNSIGNEDSHIFT);
  dgroup.setU16(nearOffsetForRuntimeSymbol("_pluy"), dgroup.u32(player + OBJ_Y_OFFSET) >>> UNSIGNEDSHIFT);
  dgroup.setU16(player + OBJ_TILEX_OFFSET, dgroup.u32(player + OBJ_X_OFFSET) >> TILESHIFT);
  dgroup.setU16(player + OBJ_TILEY_OFFSET, dgroup.u32(player + OBJ_Y_OFFSET) >> TILESHIFT);

  switch (dgroup.u16(actor + OBJ_CLASS_OFFSET)) {
    case SCHABBOBJ:
      setStateName(dgroup, actor, "_s_schabbdeathcam");
      break;
    case REALHITLEROBJ:
      setStateName(dgroup, actor, "_s_hitlerdeathcam");
      break;
    case GIFTOBJ:
      setStateName(dgroup, actor, "_s_giftdeathcam");
      break;
    case FATOBJ:
      setStateName(dgroup, actor, "_s_fatdeathcam");
      break;
  }

  return { actor, victorious: false, player, angle: playerAngle, dist, state: stateName(dgroup, actor) };
}

export function A_Victory(dgroup: DOSMemory, _actor?: number): VictorySummary {
...
export function CheckPosition(dgroup: DOSMemory, actor: number): boolean {
  const actorat = nearOffsetForRuntimeSymbol("_actorat");
  const objlist = nearOffsetForSymbol("_objlist");
  const xl = (dgroup.i32(actor + OBJ_X_OFFSET) - PLAYERSIZE) >> TILESHIFT;
  const yl = (dgroup.i32(actor + OBJ_Y_OFFSET) - PLAYERSIZE) >> TILESHIFT;
  const xh = (dgroup.i32(actor + OBJ_X_OFFSET) + PLAYERSIZE) >> TILESHIFT;
  const yh = (dgroup.i32(actor + OBJ_Y_OFFSET) + PLAYERSIZE) >> TILESHIFT;
  ...
  for (let y = yl; y <= yh; y++) {
    for (let x = xl; x <= xh; x++) {
      ...
      const check = dgroup.u16(actorat + (x * MAPSIZE + y) * 2);
...
export function ProjectileTryMove(
  dgroup: DOSMemory,
  plane0OrActor: Uint16Array | number,
  maybeActor?: number,
): boolean {
  return ProjectileTryMoveMemory(dgroup, plane0OrActor, maybeActor);
}

export function SelectPathDir(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  plane1: Uint16Array,
  actor: number,
): void {
  return SelectPathDirMemory(dgroup, plane0, plane1, actor);
}

export function SpawnAngel(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  tilex: number,
  tiley: number,
  ...
  return spawnSpearBoss(dgroup, plane0, tilex, tiley, {
    which: 17,
    state: "_s_angelstand",
    obclass: ANGELOBJ,
    options,
...
export function SpawnBJVictory(dgroup: DOSMemory, plane0: Uint16Array): BJVictorySpawnSummary {
  return SpawnBJVictoryMemory(dgroup, plane0);
}
...
export function SpawnBoss(
  ...
  return SpawnBossMemory(dgroup, plane0, tilex, tiley, options);
}

export function SpawnDeadGuard(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  tilex: number,
  tiley: number,
): EnemySpawnSummary {
  return SpawnDeadGuardMemory(dgroup, plane0, tilex, tiley);
}
...
export function SpawnFakeHitler(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  tilex: number,
  tiley: number,
  ...
  return SpawnFakeHitlerMemory(dgroup, plane0, tilex, tiley, options);
}

export function SpawnFat(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  tilex: number,
  tiley: number,
  ...
  return SpawnFatMemory(dgroup, plane0, tilex, tiley, options);
}
...
export function SpawnGhosts(
  ...
  return SpawnGhostsMemory(dgroup, plane0, which, tilex, tiley, options);
}
...
export function SpawnGift(
  ...
  return SpawnGiftMemory(dgroup, plane0, tilex, tiley, options);
}

export function SpawnGretel(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  tilex: number,
  tiley: number,
  ...
  return SpawnGretelMemory(dgroup, plane0, tilex, tiley, options);
}

export function SpawnHitler(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  tilex: number,
  tiley: number,
  ...
  return SpawnHitlerMemory(dgroup, plane0, tilex, tiley, options);
}
...
export function SpawnPatrol(
  ...
  return SpawnPatrolMemory(dgroup, plane0, which, tilex, tiley, dir, options);
}

export function SpawnSchabbs(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  tilex: number,
  tiley: number,
  ...
  return SpawnSchabbsMemory(dgroup, plane0, tilex, tiley, options);
}

export function SpawnSpectre(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  tilex: number,
  tiley: number,
  ...
  return spawnSpearBoss(dgroup, plane0, tilex, tiley, {
    which: 16,
    state: "_s_spectrewait1",
    obclass: SPECTREOBJ,
    options,
  });
}

export function SpawnStand(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  which: number,
  tilex: number,
  ...
  return SpawnStandMemory(dgroup, plane0, which, tilex, tiley, dir, options);
}

export function SpawnTrans(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  tilex: number,
  tiley: number,
...
export function T_Bite(dgroup: DOSMemory, actor: number): BiteSummary {
  return T_BiteMemory(dgroup, actor);
}

export function T_BJDone(dgroup: DOSMemory, _actor?: number): VictorySummary {
  return T_BJDoneMemory(dgroup);
}

export function T_BJJump(
  dgroup: DOSMemory,
  actor: number,
  options: { readonly tics?: number } = {},
): BJJumpSummary {
  return T_BJJumpMemory(dgroup, actor, options);
}

export function T_BJRun(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  plane1: Uint16Array,
  actor: number,
  ...
  return T_BJRunMemory(dgroup, plane0, plane1, actor, options);
}

export function T_BJYell(dgroup: DOSMemory, actor: number): SoundActionSummary {
  void dgroup;
  return positionedSound(YEAHSND, actor);
}

export function T_Chase(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  actor: number,
  options: SightPlayerOptions = {},
): ChaseStepSummary {
  return T_ChaseMemory(dgroup, plane0, actor, options);
}

export function T_DogChase(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  actor: number,
  options: SightPlayerOptions = {},
): ChaseStepSummary {
  return T_DogChaseMemory(dgroup, plane0, actor, options);
}

export function T_Fake(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  actor: number,
  options: SightPlayerOptions = {},
): ChaseStepSummary {
  return T_FakeMemory(dgroup, plane0, actor, options);
}

export function T_FakeFire(dgroup: DOSMemory, actor: number): ProjectileSpawnSummary {
  return T_FakeFireMemory(dgroup, actor);
}

export function T_Fat(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  actor: number,
  options: SightPlayerOptions = {},
): ChaseStepSummary {
  return T_FatMemory(dgroup, plane0, actor, options);
}

export function T_Ghosts(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  actor: number,
  options: SightPlayerOptions = {},
): ChaseStepSummary {
  return T_GhostsMemory(dgroup, plane0, actor, options);
}

export function T_Gift(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  actor: number,
  options: SightPlayerOptions = {},
): ChaseStepSummary {
  return T_GiftMemory(dgroup, plane0, actor, options);
}

export function T_GiftThrow(dgroup: DOSMemory, actor: number): ProjectileSpawnSummary {
  return T_GiftThrowMemory(dgroup, actor);
}

export function T_Launch(
  dgroup: DOSMemory,
  actor: number,
  options: { readonly stateName?: string } = {},
): LaunchSummary {
  let iangle = angleToPlayer(dgroup, actor);
  let shoot: ShootSummary | null = null;
  const obclass = dgroup.u16(actor + OBJ_CLASS_OFFSET);
  if (obclass === DEATHOBJ) {
    shoot = T_ShootMemory(dgroup, actor);
    if ((options.stateName ?? stateName(dgroup, actor)) === "_s_deathshoot2") {
      iangle -= 4;
      if (iangle < 0) iangle += ANGLES;
    } else {
      iangle += 4;
  ...
  const projectile = GetNewActorMemory(dgroup);
  let state = "_s_rocket";
  let projectileClass = ROCKETOBJ;
  let sound = MISSILEFIRESND;
  if (obclass === DEATHOBJ) {
    state = "_s_hrocket";
    projectileClass = HROCKETOBJ;
    sound = KNIGHTMISSILESND;
  } else if (obclass === ANGELOBJ) {
    state = "_s_spark1";
    projectileClass = SPARKOBJ;
    sound = ANGELFIRESND;
  }

  setStateName(dgroup, projectile, state);
  dgroup.setU16(projectile + OBJ_TICCOUNT_OFFSET, 1);
  dgroup.setU16(projectile + OBJ_TILEX_OFFSET, dgroup.u16(actor + OBJ_TILEX_OFFSET));
  dgroup.setU16(projectile + OBJ_TILEY_OFFSET, dgroup.u16(actor + OBJ_TILEY_OFFSET));
  dgroup.setU32(projectile + OBJ_X_OFFSET, dgroup.u32(actor + OBJ_X_OFFSET));
  dgroup.setU32(projectile + OBJ_Y_OFFSET, dgroup.u32(actor + OBJ_Y_OFFSET));
  dgroup.setU16(projectile + OBJ_CLASS_OFFSET, projectileClass);
  dgroup.setU16(projectile + OBJ_DIR_OFFSET, NODIR);
  dgroup.setU16(projectile + OBJ_ANGLE_OFFSET, iangle);
  dgroup.setU32(projectile + OBJ_SPEED_OFFSET, 0x2000);
  dgroup.setU8(projectile + OBJ_FLAGS_OFFSET, FL_NONMARK);
  dgroup.setU16(projectile + OBJ_ACTIVE_OFFSET, AC_YES);

  return {
    actor: projectile,
    source: actor,
    state,
    obclass: projectileClass,
    angle: iangle,
    speed: 0x2000,
    flags: FL_NONMARK,
    ticcount: 1,
    sound: positionedSound(sound, projectile),
    shoot,
  };
}

export function T_Path(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  plane1: Uint16Array,
  actor: number,
  ...
  return T_PathMemory(dgroup, plane0, plane1, actor, options);
}

export function T_Projectile(
  dgroup: DOSMemory,
  plane0OrActor: Uint16Array | number,
  actorOrOptions: number | { readonly tics?: number } = {},
  options: { readonly tics?: number } = {},
): ProjectileStepSummary {
  return T_ProjectileMemory(dgroup, plane0OrActor, actorOrOptions, options);
}

export function T_Schabb(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  actor: number,
  options: SightPlayerOptions = {},
): ChaseStepSummary {
  return T_SchabbMemory(dgroup, plane0, actor, options);
}

export function T_SchabbThrow(dgroup: DOSMemory, actor: number): ProjectileSpawnSummary {
  return T_SchabbThrowMemory(dgroup, actor);
}

export function T_Shoot(
  dgroup: DOSMemory,
  actor: number,
  options: SightPlayerOptions = {},
): ShootSummary {
  return T_ShootMemory(dgroup, actor, options);
}

export function T_Stand(
  dgroup: DOSMemory,
  actor: number,
  options: SightPlayerOptions = {},
): boolean {
  return T_StandMemory(dgroup, actor, options);
}

export function T_UShoot(
  dgroup: DOSMemory,
  actor: number,
  options: SightPlayerOptions = {},
): UShootSummary {
  const shoot = T_ShootMemory(dgroup, actor, options);
  const player = dgroup.u16(nearOffsetForRuntimeSymbol("_player"));
  ...
  const touchDamage = dist <= 1 ? TakeDamageMemory(dgroup, actor, 10) : null;
  return { shoot, dist, touchDamage };
}

export function T_Will(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  actor: number,
  options: SightPlayerOptions = {},
  ...
  if (CheckLineMemory(dgroup, actor, options)) {
    if (US_RndT() < (tics << 3)) {
      setStateName(dgroup, actor, willAttackState(dgroup.u16(actor + OBJ_CLASS_OFFSET)));
      return chaseSummaryLocal(dgroup, actor, false, true, false, null);
    }
    dodge = true;
  }
  ...
  if (dgroup.u16(actor + OBJ_DIR_OFFSET) === NODIR) {
    ...
    if (dgroup.u16(actor + OBJ_DIR_OFFSET) === NODIR) {
      return chaseSummaryLocal(dgroup, actor, false, false, true, null);
    }
  }
  ...
  while (move) {
    ...
    if (distance < 0) {
      ...
      if (dgroup.u16(doorobj + DOOR_ACTION_OFFSET) !== DR_OPEN) {
        return chaseSummaryLocal(dgroup, actor, moved, false, false, door);
      }
      distance = TILEGLOBAL;
    ...
    if (dgroup.u16(actor + OBJ_DIR_OFFSET) === NODIR) {
      return chaseSummaryLocal(dgroup, actor, moved, false, true, null);
    }
  }

  return chaseSummaryLocal(dgroup, actor, moved, false, false, null);
}

function dormantSummary(
  dgroup: DOSMemory,
  actor: number,
  awakened: boolean,
  blockedBy: DormantSummary["blockedBy"],
): DormantSummary {
  return {
    actor,
    awakened,
    blockedBy,
    state: stateName(dgroup, actor),
    flags: dgroup.u8(actor + OBJ_FLAGS_OFFSET),
  };
}

function spawnSpearBoss(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  tilex: number,
  tiley: number,
  ...
  const actor = spawnNewObjLocal(dgroup, plane0, tilex, tiley, config.state);
  dgroup.setU16(actor + OBJ_CLASS_OFFSET, config.obclass);
  dgroup.setU16(actor + OBJ_HITPOINTS_OFFSET, spearHitpoints(dgroup, config.which, config.options));
  dgroup.setU8(actor + OBJ_FLAGS_OFFSET, dgroup.u8(actor + OBJ_FLAGS_OFFSET) | FL_SHOOTABLE | FL_AMBUSH);
  incrementKillTotal(dgroup, config.options);
  return { actor, which: config.which, tilex, tiley, state: config.state };
}

function spawnNewObjLocal(
  dgroup: DOSMemory,
  plane0: Uint16Array,
  tilex: number,
  tiley: number,
  ...
  if (plane0.length !== MAPSIZE * MAPSIZE) {
    throw new Error(`Expected ${MAPSIZE}x${MAPSIZE} map plane`);
  }
  const actor = GetNewActorMemory(dgroup);
  setStateName(dgroup, actor, state);
  dgroup.setU16(actor + OBJ_TILEX_OFFSET, tilex);
  dgroup.setU16(actor + OBJ_TILEY_OFFSET, tiley);
  dgroup.setU32(actor + OBJ_X_OFFSET, (tilex << TILESHIFT) + TILEGLOBAL / 2);
  dgroup.setU32(actor + OBJ_Y_OFFSET, (tiley << TILESHIFT) + TILEGLOBAL / 2);
  dgroup.setU16(actor + OBJ_DIR_OFFSET, NODIR);
  dgroup.setU16(nearOffsetForRuntimeSymbol("_actorat") + (tilex * MAPSIZE + tiley) * 2, actor);
  dgroup.setU8(actor + OBJ_AREANUMBER_OFFSET, (plane0[tiley * MAPSIZE + tilex] ?? 107) - 107);
  return actor;
}

function spearHitpoints(dgroup: DOSMemory, which: number, options: EnemySpawnOptions): number {
  const difficulty = options.difficulty ?? dgroup.u16(nearOffsetForRuntimeSymbol("_gamestate"));
  const row = starthitpoints[difficulty];
  if (!row?.[which]) {
    throw new RangeError(`Invalid Spear hitpoint lookup ${difficulty}/${which}`);
  }
  return row[which];
}
...
function incrementKillTotal(dgroup: DOSMemory, options: EnemySpawnOptions): void {
  ...
  const killtotal = nearOffsetForRuntimeSymbol("_gamestate") + gamestateFieldOffset("killtotal");
  dgroup.setU16(killtotal, dgroup.u16(killtotal) + 1);
}

function setStateName(dgroup: DOSMemory, actor: number, state: string): void {
  let states = localStateNames.get(dgroup);
  if (!states) {
    states = new Map<number, string>();
    localStateNames.set(dgroup, states);
  }
...
function stateName(dgroup: DOSMemory, actor: number): string | null {
  return localStateNames.get(dgroup)?.get(actor) ?? null;
}
...
function angleToPlayer(dgroup: DOSMemory, actor: number): number {
  ...
  const deltay = dgroup.i32(actor + OBJ_Y_OFFSET) - dgroup.i32(player + OBJ_Y_OFFSET);
  return angleFromDelta(deltax, deltay);
}

function angleFromDelta(deltax: number, deltay: number): number {
...
function willAttackState(obclass: number): string {
  if (obclass === WILLOBJ) {
    return "_s_willshoot1";
  }
  if (obclass === ANGELOBJ) {
    return "_s_angelshoot1";
  }
  return "_s_deathshoot1";
}

function chaseSummaryLocal(
  dgroup: DOSMemory,
  actor: number,
  moved: boolean,
  attacked: boolean,
