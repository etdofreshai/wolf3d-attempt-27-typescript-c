...
|---|------|---------------------------|
| 1 | **Global/`extern` mechanism** (§6.4) | Each global is **defined in the `.ts` mirror of the `.c` that defines it** — never a catch-all globals module (that would destroy the 1:1 structure). Serialized / pointer-addressed globals are **views into the DOS-memory buffer** (§6.3). *Mechanism (locked, refined from the original sketch):* ESM namespace objects are **frozen** (`IN.tics = x` throws), so writable-from-anywhere scalars can't be live module bindings. Instead **every mutable C global lives on a per-file exported state record** named by the file's stem abbreviation (`export const ca = {...}` in `ID_CA.C.ts`; access `ca.mapon`, foreign or local). This also gives `MM_GetPtr(&ptr)`-style out-params a stable identity (`ref(ca,'grstarts')` ≈ `&grstarts` — see `runtime/dosmem.ts` PtrCell). Const tables stay plain exports. |
