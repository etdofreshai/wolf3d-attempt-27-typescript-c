...
const tempDir=await mkdtemp(path.join(os.tmpdir(),"wsn-")); const entry=path.join(tempDir,"e.ts"); const out=path.join(tempDir,"e.mjs");
...
const s=mod.PlayDemoTrace(db,new Uint16Array(p0),new Uint16Array(p1),dg,{areaconnect:new Uint8Array(37*37),sampleEvery:1});
for(const t of s.trace){ if(t.commandIndex>=1108 && t.commandIndex<=1124) console.log(`cmd ${t.commandIndex}: soundPlaying=${t.soundPlaying} pcLen=${t.pcLengthLeft} pcActive=${t.pcSoundActive} sndMode=${t.soundMode} weaponframe=${t.weaponframe} attackcount=${t.attackcount}`); }
