import { mkdtemp, readFile, readFileSync, rm, writeFile } from "node:fs";
import { mkdtemp as mkdtempP, readFile as readFileP, rm as rmP, writeFile as writeFileP } from "node:fs/promises";
...
const TIC=Number(process.argv[2]??563); // oracle block index = after command TIC ; port runTo(TIC+1)
const buf=readFileSync("tmp/oracle-build/ORTRACE.BIN");
const OBJSIZE=60,MAXACTORS=150,SPOTVIS=64*64,PIXEL=320*2,BLOCK=MAXACTORS*OBJSIZE+SPOTVIS+PIXEL;
const ospot=(x,y)=>buf[TIC*BLOCK+MAXACTORS*OBJSIZE+x*64+y];
const tempDir=await mkdtempP(path.join(os.tmpdir(),"wsd-")); const entry=path.join(tempDir,"e.ts"); const out=path.join(tempDir,"e.mjs");
const rel=(p)=>{let r=path.relative(tempDir,p).split(path.sep).join("/");return r.startsWith(".")?r:"./"+r;};
await writeFileP(entry,[
...
for(const n of names) files[n]=new Uint8Array(await readFileP(path.join(wl6Dir,`${n}.WL6`)));
...
const dg=new mod.DOSMemory(0x10000); mod.PlayDemoTrace(db,new Uint16Array(p0),new Uint16Array(p1),dg,{areaconnect:new Uint8Array(37*37),sampleEvery:1,maxCommands:TIC+1});
const sv=mod.nearOffsetForRuntimeSymbol("_spotvis");
let diffs=[]; let portOnly=0, oracOnly=0;
for(let x=0;x<64;x++)for(let y=0;y<64;y++){const p=dg.u8(sv+x*64+y)?1:0;const o=ospot(x,y)?1:0;if(p!==o){diffs.push(`${x},${y}:port=${p} orac=${o}`);if(p&&!o)portOnly++;else oracOnly++;}}
console.log(`TIC=${TIC} spotvis diffs: ${diffs.length} (portOnly=${portOnly} oracOnly=${oracOnly})`);
diffs.slice(0,60).forEach(d=>console.log("  "+d));
await rmP(tempDir,{recursive:true,force:true});
