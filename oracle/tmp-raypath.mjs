// Trace the FULL tile path of a single pixx column in the port's final refresh at cmd 563.
// Watches all tiles, filters the RAY_DEBUG log to the requested pixx.
...
const tempDir = await mkdtemp(path.join(os.tmpdir(), "wolf-rpath-"));
...
mod.RAY_DEBUG.tiles = new Set(Array.from({ length: 4096 }, (_, i) => i));
mod.RAY_DEBUG.log = [];
...
let maxIdx = 0;
for (const e of mod.RAY_DEBUG.log) if (e.refreshIdx > maxIdx) maxIdx = e.refreshIdx;
const tileOff = mod.nearOffsetForRuntimeSymbol("_tilemap");
const tile = (x, y) => dgroup.u8(tileOff + x * 64 + y);
const entries = mod.RAY_DEBUG.log.filter((e) => e.refreshIdx === maxIdx && e.pixx === PIXX);
console.log(`pixx=${PIXX} final refresh path (${entries.length} marked tiles), tilemap value at each:`);
for (const e of entries) {
  const [x, y] = e.tile;
  console.log(`  mark(${x},${y}) ${e.side} tile=${tile(x,y)} | xint=${e.xintercept} yint=${e.yintercept} xtile=${e.xtile} ytile=${e.ytile} xinttile=${e.xinttile} yinttile=${e.yinttile}`);
}
