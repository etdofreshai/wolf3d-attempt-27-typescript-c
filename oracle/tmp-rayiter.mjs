// Dump per-iteration DDA decisions for one pixx at one refresh.
...
const PIXX = Number(process.argv[3] ?? "217");

const tempDir = await mkdtemp(path.join(os.tmpdir(), "wolf-ri-"));
...
// RefreshDemoVisibility increments refreshIdx each command; final refresh idx == M.
mod.RAY_DEBUG.active = true;
mod.RAY_DEBUG.tiles = new Set();
mod.RAY_DEBUG.log = [];
mod.RAY_DEBUG.refreshIdx = 0;
mod.RAY_DEBUG.onlyPixx = PIXX;
mod.RAY_DEBUG.onlyRefresh = M; // refreshIdx increments to M over M commands
mod.RAY_DEBUG.iters = [];
...
console.log(`final refreshIdx reached: ${mod.RAY_DEBUG.refreshIdx}; iters captured: ${mod.RAY_DEBUG.iters.length}`);
for (const it of mod.RAY_DEBUG.iters) {
  if (it.k === "init") {
    console.log(`INIT xtile=${it.xtile} ytile=${it.ytile} yinttile=${it.yinttile} xinttile=${it.xinttile} xint=${it.xintercept} yint=${it.yintercept} xstep=${it.xstep} ystep=${it.ystep} xts=${it.xtilestep} yts=${it.ytilestep} focal=(${it.focaltx},${it.focalty})`);
  } else {
    console.log(`${it.k.padEnd(5)} entry=${it.entry.padEnd(10)} xtile=${it.xtile} ytile=${it.ytile} yinttile=${it.yinttile} xinttile=${it.xinttile} check=(${it.check[0]},${it.check[1]}) hit=${it.hit} | xint=${it.xintercept} yint=${it.yintercept}`);
  }
}
